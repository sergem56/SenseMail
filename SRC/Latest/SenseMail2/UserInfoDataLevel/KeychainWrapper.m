...
- (instancetype)init
{
    ...
    if (self) {
        ...
        /*
         ...
         NULL);
         [_genericPasswordQuery setObject:(__bridge id _Nonnull)(access) forKey:(__bridge id)kSecAttrAccessControl];
         */
...
- (void)mySetObject:(id)inObject forKey:(id)key forAccount:(NSString*)account
{
    ...
    if (![currentObject isEqual:inObject])
    {
        ...
        // To enable saving multiple values
...
// This one reads the last recorded value.
...
- (NSString*)getKeychainDataForAccount:(NSString*)key
{
    ...
    if(status == noErr) {
        NSData* res = CFBridgingRelease(result);
        NSString* ret = [[NSString alloc] initWithBytes:[res bytes]
                                                 length:[res length] encoding:NSUTF8StringEncoding];
        return ret;
    } else {
