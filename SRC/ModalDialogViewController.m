...
static NSString* retText1;
static NSString* retText2;
...
+(NSString*)runWithHeader:(NSString*)header text1:(NSString*)text1 text2:(NSString*)text2 block:(dispatch_block_t)dblock;
{
    ...
    UILabel* headerLabel = (UILabel*)[windowController.view viewWithTag:1];
    dispatch_async(dispatch_get_main_queue(), ^{
        headerLabel.text = header;
    });
    
    UILabel* text1Label = (UILabel*)[windowController.view viewWithTag:2];
    dispatch_async(dispatch_get_main_queue(), ^{
        text1Label.text = text1;
    });
    
    UILabel* text2Label = (UILabel*)[windowController.view viewWithTag:4];
    dispatch_async(dispatch_get_main_queue(), ^{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [[[GlobalRouter sharedManager] getNavController] presentViewController:windowController animated:YES completion:nil];
    });
...
-(IBAction)ok:(id)sender
{
    [self dismissViewControllerAnimated:YES completion:^{
        retText1 = self.text1.text;
        retText2 = self.text2.text;
